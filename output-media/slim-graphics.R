...
for (file in 1:length(p1.files)) {
    ...
    par(mfrow=c(3,1))
    plot(mean.phenotype~gen, data = rep0.data , type = "l", cex = 3.5, col = "blue", ylim = c(-20,20))
...
# make plots for p3
for (file in 1:length(p3.files.1)) {
    print(file)
    rep.0 = p3.files.0[file]
    rep.1 = p3.files.1[file]
    print(rep.0)
    print(rep.1)
    rep0.data <- read.csv(paste(dir,"/0/",rep.0,sep = ""), header = TRUE)
...
#     if (p3.data[file.size(p3.data)>71]){
